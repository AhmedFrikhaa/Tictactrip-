import { generateToken, trackWordUsage, justifyText } from '../app';

describe('Token and Justification Tests', () => {
    let token: string;

    beforeAll(() => {
        token = generateToken('test@example.com');
    });
    ...
    it('should track word usage and not exceed limit', () => {
        const success = trackWordUsage(token, 100);
        expect(success).toBe(true);
    });

    it('should return false if word usage exceeds limit', () => {
        const exceeded = trackWordUsage(token, 80000);
